...
SAMPLE_LEADS = [
    {"id":"1","nombre":"Carlos Mendoza","empresa":"Mendoza Holdings","telefono":"+34 666 111 222","email":"c.mendoza@mendozaholdings.com","idioma":"Español","tipoEmbarcacion":"Motor","modeloEslora":"Sunseeker Predator 74'","presupuesto":1200000,"usoPrevisto":"Uso personal Mediterráneo","asignadoA":"Vendedor 1","etapa":"Negociación","probabilidad":75,"valorOperacion":1150000,"fuenteLead":"Feria Náutica","proximaAccion":"Enviar propuesta financiación","fechaProximaAccion":str(date.today()-timedelta(days=1)),"historial":[{"fecha":"2024-03-10","tipo":"Email","nota":"Primer contacto feria METS."},{"fecha":"2024-04-02","tipo":"Reunión","nota":"Visita astillero. Solicita propuesta."}],"fechaCreacion":"2024-03-10","ultimaActualizacion":"2024-04-02"},
    {"id":"2","nombre":"Marie Dupont","empresa":"Dupont Family Office","telefono":"+33 6 12 34 56 78","email":"marie@dupontfo.fr","idioma":"Francés","tipoEmbarcacion":"Velero","modeloEslora":"Jeanneau Sun Odyssey 490","presupuesto":280000,"usoPrevisto":"Regatas y cruceros","asignadoA":"Vendedor 2","etapa":"Propuesta Enviada","probabilidad":55,"valorOperacion":265000,"fuenteLead":"Web","proximaAccion":"Seguimiento propuesta","fechaProximaAccion":str(date.today()),"historial":[{"fecha":"2024-03-20","tipo":"Email","nota":"Solicitud desde la web."},{"fecha":"2024-04-05","tipo":"Email","nota":"Propuesta enviada."}],"fechaCreacion":"2024-03-20","ultimaActualizacion":"2024-04-05"},
    {"id":"3","nombre":"Roberto Ferretti","empresa":"Ferretti SpA","telefono":"+39 02 1234 5678","email":"r.ferretti@ferrettispa.it","idioma":"Italiano","tipoEmbarcacion":"Catamarán","modeloEslora":"Beneteau Excess 15","presupuesto":450000,"usoPrevisto":"Charter Sicilia","asignadoA":"Vendedor 1","etapa":"Interés Confirmado","probabilidad":40,"valorOperacion":430000,"fuenteLead":"Referido","proximaAccion":"Enviar ficha técnica","fechaProximaAccion":str(date.today()+timedelta(days=2)),"historial":[{"fecha":"2024-04-01","tipo":"Llamada","nota":"Referido por cliente anterior."}],"fechaCreacion":"2024-04-01","ultimaActualizacion":"2024-04-08"},
    {"id":"4","nombre":"Ana Sousa","empresa":"Sousa & Partners","telefono":"+351 91 234 5678","email":"ana.sousa@sousapartners.pt","idioma":"Portugués","tipoEmbarcacion":"Motor","modeloEslora":"Princess V55","presupuesto":650000,"usoPrevisto":"Uso familiar Algarve","asignadoA":"Vendedor 3","etapa":"Contactado","probabilidad":25,"valorOperacion":620000,"fuenteLead":"RRSS","proximaAccion":"Llamada cualificación","fechaProximaAccion":str(date.today()+timedelta(days=3)),"historial":[{"fecha":"2024-04-10","tipo":"Email","nota":"Contacto desde Instagram."}],"fechaCreacion":"2024-04-10","ultimaActualizacion":"2024-04-10"},
    {"id":"5","nombre":"Thomas Weber","empresa":"Weber Yachting GmbH","telefono":"+49 89 1234 5678","email":"t.weber@weberyachting.de","idioma":"Alemán","tipoEmbarcacion":"Velero","modeloEslora":"Beneteau Oceanis 46.1","presupuesto":310000,"usoPrevisto":"Circunnavegación","asignadoA":"Vendedor 2","etapa":"Cerrado Ganado","probabilidad":100,"valorOperacion":298000,"fuenteLead":"Feria Náutica","proximaAccion":"Coordinar entrega Palma","fechaProximaAccion":str(date.today()+timedelta(days=30)),"historial":[{"fecha":"2024-02-15","tipo":"Llamada","nota":"Contacto Boot Düsseldorf."},{"fecha":"2024-03-20","tipo":"Email","nota":"Contrato firmado."}],"fechaCreacion":"2024-02-15","ultimaActualizacion":"2024-03-20"},
]
...
if "leads"        not in st.session_state: st.session_state.leads        = SAMPLE_LEADS.copy()
if "vendedores"   not in st.session_state: st.session_state.vendedores   = ["Vendedor 1","Vendedor 2","Vendedor 3"]
if "archivo_frio" not in st.session_state: st.session_state.archivo_frio = []
if "boat_types"   not in st.session_state: st.session_state.boat_types   = ["Velero","Motor","Catamarán","Zodiac","Charter","Jeanneau","Beneteau","Sunseeker","Princess","Azimut","Ferretti","Bavaria","Hanse","Lagoon","Otro"]

activos_nuevos,nuevos_archivados=auto_archivar(st.session_state.leads)
if nuevos_archivados:
    st.session_state.leads=activos_nuevos
    st.session_state.archivo_frio.extend(nuevos_archivados)
...
if MODO_FERIA:
    ...
    with st.form("feria_form", clear_on_submit=True):
        ...
        tipo     = st.selectbox("🚢 Tipo / Marca de embarcación", st.session_state.boat_types)
        modelo   = st.text_input("📐 Modelo / Eslora")
        presu    = st.number_input("💶 Presupuesto aprox. (€)", min_value=0, step=10000)
        asig     = st.selectbox("👔 Asignar a", st.session_state.vendedores)
        nota_ini = st.text_area("📝 Nota inicial", placeholder="Observaciones del contacto...", height=80)
        ...
        if ok:
            if not nombre.strip():
                ...
                st.session_state.leads.append({
                    "id":str(uuid.uuid4()),"nombre":nombre,"empresa":empresa,
                    "telefono":telefono,"email":email,"idioma":idioma,
                    "tipoEmbarcacion":tipo,"modeloEslora":modelo,
                    "presupuesto":int(presu),"usoPrevisto":"",
                    "asignadoA":asig,"etapa":"Prospecto","probabilidad":10,
                    "valorOperacion":int(presu),"fuenteLead":"Feria Náutica",
                    "proximaAccion":"Contactar post-feria",
                    "fechaProximaAccion":str(date.today()+timedelta(days=3)),
                    "historial":[{"fecha":str(date.today()),"tipo":"Nota","nota":nota_ini.strip()}] if nota_ini.strip() else [],
                    "fechaCreacion":str(date.today()),"ultimaActualizacion":str(date.today()),
                })
                st.success(f"✅ **{nombre}** guardado como Prospecto.")
                st.balloons()
    st.markdown("---")
    recientes=[l for l in st.session_state.leads if l.get("fuenteLead")=="Feria Náutica"][-5:]
    if recientes:
        st.caption(f"Últimos {len(recientes)} leads de feria registrados:")
...
with st.sidebar:
    ...
    st.caption("🟡 Versión DEMO — datos en memoria")
    st.markdown("---")
    active_user  = st.selectbox("👤 Usuario activo", st.session_state.vendedores)
    ...
    st.markdown("---")
    n_arch=len(st.session_state.archivo_frio)
    if n_arch>0:
        st.markdown(f"<div style='background:#1a3050;border-radius:6px;padding:6px 10px;font-size:0.75rem;color:#7a8fa6'>🧊 Archivo Frío: <b style='color:#e8e0d0'>{n_arch}</b></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("<a href='?modo=feria' target='_blank' style='background:#c9a84c;color:#0a1628;padding:8px 14px;border-radius:6px;font-weight:700;font-size:0.8rem;text-decoration:none'>📱 Abrir modo Feria</a>", unsafe_allow_html=True)
    st.caption("\nDatos se reinician al cerrar la pestaña.")

leads     = st.session_state.leads
vendedores= st.session_state.vendedores
...
if "Kanban" in page:
    ...
    with st.expander("⚡ Cambio rápido de estado"):
        ...
        nombres_k=[l["nombre"] for l in st.session_state.leads if l["etapa"] not in ["Cerrado Ganado","Cerrado Perdido"]]
        ...
        if c3.button("✅ Cambiar"):
            if sel_k!="— Selecciona —":
                for i,l in enumerate(st.session_state.leads):
                    if l["nombre"]==sel_k:
                        st.session_state.leads[i]["etapa"]=nueva_etapa
                        st.session_state.leads[i]["ultimaActualizacion"]=str(date.today())
                        break
    ...
    for col,stage in zip([bc1,bc2,bc3],["Cerrado Ganado","Cerrado Perdido","En Pausa / Recuperable"]):
        cards=[l for l in st.session_state.leads if l["etapa"]==stage]
        total=sum(l.get("valorOperacion",0) for l in cards)
        color=STAGE_COLORS[stage]
...
# ══ LISTA ════════════════════════════════════════════════════════════════════
    ...
    ft=c4.selectbox("Tipo",["Todos"]+st.session_state.boat_types)
    filtered=leads
...
# ══ INFORMES ═════════════════════════════════════════════════════════════════
    ...
    active=[l for l in all_leads if l["etapa"] not in ["Cerrado Ganado","Cerrado Perdido","En Pausa / Recuperable"]]
    pipeline=sum(l.get("valorOperacion",0) for l in active)
    forecast=sum(l.get("valorOperacion",0)*l.get("probabilidad",0)/100 for l in all_leads if l["etapa"]!="Cerrado Perdido")
    no_act=[l for l in active if days_since(l.get("ultimaActualizacion") or l.get("fechaCreacion",""))>7]
    k1,k2,k3,k4=st.columns(4)
    k1.metric("Total Leads",len(all_leads),f"{len(active)} activos")
    ...
    with tab1:
        st.markdown("#### Embudo de Ventas — Leads e Importes")
        funnel_data=[{"Etapa":s,"Leads":len([l for l in all_leads if l["etapa"]==s]),"Valor":sum(l.get("valorOperacion",0) for l in all_leads if l["etapa"]==s),"Color":STAGE_COLORS[s]} for s in FUNNEL_STAGES]
        c1,c2=st.columns(2)
        with c1:
            st.markdown("**Por número de leads**")
            fig1=go.Figure(go.Funnel(y=[d["Etapa"] for d in funnel_data],x=[d["Leads"] for d in funnel_data],textinfo="value+percent initial",marker=dict(color=[d["Color"] for d in funnel_data]),textfont=dict(color="white",size=12)))
        ...
        with c2:
            ...
            fig2=go.Figure(go.Funnel(y=[d["Etapa"] for d in funnel_data],x=[d["Valor"] for d in funnel_data],texttemplate=[fmt_eur(d["Valor"])+"<br>%{percentInitial}" for d in funnel_data],marker=dict(color=[d["Color"] for d in funnel_data]),textfont=dict(color="white",size=11)))
        ...
        rows_f=[{"Etapa":d["Etapa"],"Leads":d["Leads"],"Valor Pipeline":fmt_eur(d["Valor"]),"Ticket Medio":fmt_eur(d["Valor"]/d["Leads"]) if d["Leads"]>0 else "—"} for d in funnel_data]
    ...
    with tab2:
        ...
        with c2:
            ...
            src={}
            for l in all_leads: src[l.get("fuenteLead","Otro")]=src.get(l.get("fuenteLead","Otro"),0)+1
        ...
        ic={}
        for l in all_leads: ic[l.get("idioma","—")]=ic.get(l.get("idioma","—"),0)+1
    ...
    st.markdown("---")
    csv_inf=pd.DataFrame([{k:v for k,v in l.items() if k!="historial"} for l in all_leads]).to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Exportar todos los leads CSV",data=csv_inf,file_name="nauticrm_informe.csv",mime="text/csv")

# ══ LEAD FORM ═════════════════════════════════════════════════════════════════
elif "Lead" in page:
    st.markdown("## ➕ Nuevo / Editar Lead")
    nombres_lista=["— Crear nuevo lead —"]+[l["nombre"] for l in st.session_state.leads]
    sel=st.selectbox("Seleccionar lead existente",nombres_lista)
    existing=None if sel=="— Crear nuevo lead —" else next((l for l in st.session_state.leads if l["nombre"]==sel),None)
    d=existing or {}
    ...
    if existing:
        ...
        etapa_rapida=c1.selectbox("Nueva etapa",STAGES,index=STAGES.index(existing["etapa"]) if existing["etapa"] in STAGES else 0,key="etapa_rapida")
        if c2.button("✅ Actualizar"):
            idx=next(i for i,l in enumerate(st.session_state.leads) if l["id"]==existing["id"])
            st.session_state.leads[idx]["etapa"]=etapa_rapida
            st.session_state.leads[idx]["ultimaActualizacion"]=str(date.today())
            st.success(f"✅ → {etapa_rapida}"); st.rerun()
        st.markdown("---")
    ...
    with st.form("lead_form"):
        ...
        bt=st.session_state.boat_types
        c6,c7,c8=st.columns(3)
        tipo  =c6.selectbox("Tipo / Marca",bt,index=bt.index(d["tipoEmbarcacion"]) if d.get("tipoEmbarcacion") in bt else 0)
        ...
        asig  =c10.selectbox("Asignado a",st.session_state.vendedores,index=st.session_state.vendedores.index(d["asignadoA"]) if d.get("asignadoA") in st.session_state.vendedores else 0)
        ...
        if submitted:
            if not nombre.strip(): st.error("El nombre es obligatorio.")
                ...
                if existing:
                    idx=next(i for i,l in enumerate(st.session_state.leads) if l["id"]==existing["id"])
                    st.session_state.leads[idx]=new_lead; st.success("✅ Lead actualizado.")
                else:
                    st.session_state.leads.append(new_lead); st.success("✅ Lead creado.")
                st.rerun()

    if existing:
        st.markdown("---")
        if st.button("🗑️ Eliminar este lead"):
            st.session_state.leads=[l for l in st.session_state.leads if l["id"]!=existing["id"]]
            st.success("Lead eliminado."); st.rerun()
...
# ══ PRÓXIMAS ACCIONES ════════════════════════════════════════════════════════
    ...
    acciones=[l for l in all_leads if (l.get("proximaAccion") or l.get("fechaProximaAccion")) and l["etapa"] not in ["Cerrado Ganado","Cerrado Perdido"]]
    ...
    sin_accion=[l for l in all_leads if not l.get("proximaAccion") and not l.get("fechaProximaAccion") and l["etapa"] not in ["Cerrado Ganado","Cerrado Perdido","En Pausa / Recuperable"]]
    k1,k2,k3,k4=st.columns(4)
    ...
    if sin_accion:
        st.markdown("---"); st.markdown("### ⚪ Sin próxima acción asignada")
        for l in sin_accion:
            color=STAGE_COLORS.get(l["etapa"],"#1a3050")
...
# ══ ARCHIVO FRÍO ══════════════════════════════════════════════════════════════
    ...
    st.markdown("Contactos en pausa más de **6 meses**. Disponibles para reactivación y campañas.")
    pausados=[l for l in st.session_state.leads if l.get("etapa")=="En Pausa / Recuperable"]
    if pausados:
        with st.expander(f"📥 Archivar manualmente ({len(pausados)} en pausa)"):
            for l in pausados:
                ...
                if c2.button("Archivar",key=f"arch_{l['id']}"):
                    st.session_state.leads=[x for x in st.session_state.leads if x["id"]!=l["id"]]
                    st.session_state.archivo_frio.append({**l,"fechaArchivo":str(date.today())}); st.rerun()
    archivo=st.session_state.archivo_frio
    if not archivo: st.info("El archivo frío está vacío.")
    else:
        c1,c2,c3=st.columns(3)
        q_a=c1.text_input("🔍 Buscar",key="q_arch"); ft_a=c2.selectbox("Tipo",["Todos"]+st.session_state.boat_types,key="ft_arch"); fv_a=c3.selectbox("Vendedor",["Todos"]+vendedores,key="fv_arch")
        filtrado=archivo
        ...
        st.caption(f"{len(filtrado)} de {len(archivo)} contactos")
        ...
        sel_r=st.selectbox("Seleccionar",["— Selecciona —"]+[l["nombre"] for l in archivo])
        if sel_r!="— Selecciona —" and st.button("♻️ Reactivar como Prospecto"):
            lr=next(l for l in archivo if l["nombre"]==sel_r)
            st.session_state.archivo_frio=[l for l in archivo if l["nombre"]!=sel_r]
            st.session_state.leads.append({**lr,"etapa":"Prospecto","ultimaActualizacion":str(date.today())})
...
# ══ CONFIG ════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        with st.form("cfg_v"):
            names=[st.text_input(f"Vendedor {i+1}",value=v) for i,v in enumerate(st.session_state.vendedores)]
            if st.form_submit_button("💾 Guardar nombres"):
                st.session_state.vendedores=names; st.success("✅ Actualizado."); st.rerun()
    with tab2:
        st.caption("Gestiona los valores del desplegable de embarcación en la ficha de lead.")
        st.markdown("<br>", unsafe_allow_html=True)
        cols_bt=st.columns(4); to_delete=None
        for i,bt in enumerate(st.session_state.boat_types):
            with cols_bt[i%4]:
                c1,c2=st.columns([3,1])
        ...
        if to_delete:
            st.session_state.boat_types=[x for x in st.session_state.boat_types if x!=to_delete]; st.rerun()
        st.markdown("<br>", unsafe_allow_html=True)
        with st.form("cfg_b"):
            c1,c2=st.columns([3,1])
            ...
            if c2.form_submit_button("➕ Añadir"):
                if nueva.strip() and nueva.strip() not in st.session_state.boat_types:
                    st.session_state.boat_types.append(nueva.strip()); st.success(f"✅ '{nueva.strip()}' añadido."); st.rerun()
                elif nueva.strip() in st.session_state.boat_types: st.warning("Ya existe.")
        if st.button("↺ Restaurar por defecto"):
            st.session_state.boat_types=["Velero","Motor","Catamarán","Zodiac","Charter","Jeanneau","Beneteau","Sunseeker","Princess","Azimut","Ferretti","Bavaria","Hanse","Lagoon","Otro"]; st.rerun()
    st.markdown("---")
    st.markdown("##### 📱 Enlace Modo Feria")
    st.code("http://localhost:8501/?modo=feria")
    st.caption("En Streamlit Cloud será: https://tu-app.streamlit.app/?modo=feria")
    st.markdown("<br>", unsafe_allow_html=True)
    st.info("🟡 Versión DEMO — datos en memoria. En la versión final se guardan en Google Sheets.")

        ...
        with col:
            ...
            for l in cards:
                ...
                st.markdown(f"""<div style="background:#0d1e35;border:1px solid #1a3050;border-left:3px solid {color};border-radius:6px;padding:7px 9px;margin:4px 0">
                    <div style="font-weight:700;font-size:0.75rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{urg_emoji(l.get('fechaProximaAccion'))} <span style="color:#c9a84c;cursor:pointer" title="Ver ficha">{l['nombre']}</span></div>
                ...
                if st.button(f"👁 {l['nombre']}", key=f"kb_{l['id']}", help="Ir a ficha"):
        ...
        if not fd:
            ...
            with c1:
                ...
                fig1=go.Figure(go.Funnel(
                    ...
                    textinfo="value",
                    texttemplate=[f"{d['Leads']} leads" for d in fd],
                    textfont=dict(color="white",size=13),
                    marker=dict(color=[d["Color"] for d in fd],line=dict(color="rgba(0,0,0,0.2)",width=1)),
                    connector=dict(line=dict(color="#1a3050",width=1,dash="dot")),
                ))
                fig1.update_layout(
            ...
            with c2:
                ...
                fig2=go.Figure(go.Funnel(
                    ...
                    textinfo="value",
                    texttemplate=[fmt_eur(d["Valor"]) for d in fd],
                    textfont=dict(color="white",size=13),
                    marker=dict(color=[d["Color"] for d in fd],line=dict(color="rgba(0,0,0,0.2)",width=1)),
                    connector=dict(line=dict(color="#1a3050",width=1,dash="dot")),
                ))

        ...
        with col:
            ...
            for l in cards:
                ...
                st.markdown(f"""<div style="background:#0d1e35;border:1px solid #1a3050;border-left:3px solid {color};border-radius:6px;padding:7px 9px;margin:4px 0">
                    <div style="font-weight:700;font-size:0.75rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{urg_emoji(l.get('fechaProximaAccion'))} <span style="color:#c9a84c">{l['nombre']}</span></div>
                    <div style="color:#7a8fa6;font-size:0.65rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:3px">{l.get('modeloEslora') or l['tipoEmbarcacion']}</div>
                ...
                # Botón invisible sobre la tarjeta como enlace
                st.markdown(f"<style>div[data-testid='stButton'] button[title='{l['nombre']}']{{position:relative;margin-top:-62px;width:100%;height:60px;opacity:0;cursor:pointer}}</style>", unsafe_allow_html=True)
                if st.button(l['nombre'], key=f"kb_{l['id']}", help=l['nombre']):
        ...
        if not fd:
            ...
            with c1:
                ...
                fig1=go.Figure(go.Bar(
                    ...
                    x=[d["Leads"] for d in fd],
                    orientation="h",
                    text=[f"{d['Leads']} leads" for d in fd],
                    textposition="inside",
            ...
            with c2:
                ...
                fig2=go.Figure(go.Bar(
                    y=[d["Etapa"] for d in fd],
                    x=[d["Valor"] for d in fd],
                    orientation="h",
                    text=[fmt_eur(d["Valor"]) for d in fd],
                    textposition="inside",
                    textfont=dict(color="white",size=12),
                    marker=dict(color=[d["Color"] for d in fd],line=dict(color="rgba(0,0,0,0.15)",width=1)),
                ))

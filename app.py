...
# ══ INFORMES ══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fd=[d for d in fd if d["Leads"]>0 or d["Valor"]>0]  # evitar Infinity%
        c1,c2=st.columns(2)
        with c1:
        ...
        with c2:
            ...
            fig2=go.Figure(go.Funnel(y=[d["Etapa"] for d in fd],x=[d["Valor"] for d in fd],texttemplate=[fmt_eur(d["Valor"]) for d in fd],marker=dict(color=[d["Color"] for d in fd]),textfont=dict(color="white",size=11)))
